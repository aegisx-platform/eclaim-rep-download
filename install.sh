...
# Ask for external database settings if --no-db mode
if [ "$DB_TYPE" = "none" ]; then
    echo ""
    echo -e "${BLUE}กรุณาใส่ข้อมูล Database ภายนอก:${NC}"
    echo ""
    echo -e "เลือก Database Type:"
    echo -e "  1) PostgreSQL (default)"
    echo -e "  2) MySQL"
    read -p "เลือก [1/2]: " DB_CHOICE </dev/tty
    ...
    read -p "DB_HOST: " EXT_DB_HOST </dev/tty
    read -p "DB_PORT [$DEFAULT_PORT]: " EXT_DB_PORT </dev/tty
    EXT_DB_PORT=${EXT_DB_PORT:-$DEFAULT_PORT}
    read -p "DB_NAME [eclaim_db]: " EXT_DB_NAME </dev/tty
    EXT_DB_NAME=${EXT_DB_NAME:-eclaim_db}
    read -p "DB_USER: " EXT_DB_USER </dev/tty
    read -s -p "DB_PASSWORD: " EXT_DB_PASS </dev/tty
    echo ""
fi
...
# Add external database settings if --no-db mode
if [ "$DB_TYPE" = "none" ]; then
    cat >> .env << EOF
